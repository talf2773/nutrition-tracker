...
// Proxy to Google Gemini API (free tier: 1500 requests/month)
...
Be specific with amounts based on what you see. Identify each component separately.`;
  ...
  try {
    ...
    if (!geminiRes.ok) {
      ...
      return res.status(geminiRes.status).json({ error: data.error?.message || 'Gemini error', quota_exceeded: geminiRes.status === 429 });
    }
    ...
    try {
      ...
      return res.status(200).json({ ok: false, raw: text, error: 'Could not parse Gemini response as JSON' });
    }
